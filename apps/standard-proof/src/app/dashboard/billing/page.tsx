...
export default async function BillingPage({ searchParams }: BillingPageProps) {
  ...
  if (!isLocalGatewayMode() && ownerId) {
    ...
    const [collection] = await db
      ...
      .limit(1);
    if (!billing && collection?.plan) currentPlan = collection.plan;
  } else if (isLocalGatewayMode()) {
    currentPlan = "free";
  }

  const plan = getPlan(PRODUCT, currentPlan as "free" | "starter" | "growth");
  const paidPlans = getPaidPlans(PRODUCT);

  return (
    <>
      <h1 className="ms-app-title">Billing</h1>
      <p className="mt-2 ms-app-muted">Manage your Standard Proof subscription.</p>
      ...
      <Card className="mt-8">
        ...
        <CardContent className="space-y-4">
          <ul className="text-sm ms-app-muted">
            <li>Collections: {String(plan.limits.collections)}</li>
            <li>Testimonials: {String(plan.limits.testimonials)}</li>
        ...
        {paidPlans.map((tier) => {
          ...
          return (
            <Card key={tier.tier}>
              ...
              <CardContent className="space-y-4">
                <ul className="text-sm ms-app-muted">
                  <li>{String(tier.limits.collections)} collections</li>
                  <li>{String(tier.limits.testimonials)} testimonials</li>
