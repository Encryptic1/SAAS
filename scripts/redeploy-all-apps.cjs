/**
 * Trigger production redeployments of all 14 apps via the Vercel API,
 * using the gitSource so Vercel builds from the latest commit on main.
 *
...
async function main() {
  ...
  for (const app of APPS) {
    ...
    if (res.status === 200 || res.status === 201) {
      ...
      console.log(`  FAIL ${app.padEnd(22)} status=${res.status} ${res.text.slice(0, 180)}`);
    }
  }
  ...
  console.log(`\n--- Polling ${triggered.length} deployments until READY (timeout 15 min) ---`);
